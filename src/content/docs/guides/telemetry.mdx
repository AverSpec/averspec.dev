...
<TabItem label="Ruby">
```ruby
action :assign_task, telemetry: ->(p) {
  { span: "task.assign",
    attributes: { "task.title" => p.title },
    causes: ["notification.process"] }
}
...
<TabItem label="Go">
```go
AssignTask: aver.Action[AssignTaskPayload](aver.Telemetry(func(p AssignTaskPayload) aver.TelemetryExpectation {
    return aver.TelemetryExpectation{
        Span:       "task.assign",
        Attributes: map[string]any{"task.title": p.Title},
        Causes:     []string{"notification.process"},
    }
})),
```
</TabItem>
<TabItem label="Rust">
```rust
assign_task: action::<AssignTaskPayload>()
    .telemetry(|p| Telemetry::new("task.assign")
        .attr("task.title", &p.title)
        .causes(["notification.process"]))
    .build(),
```
</TabItem>
...
```kotlin
val assignTask = action<AssignTaskPayload>("assignTask",
    telemetry = { p -> TelemetryExpectation(
        span = "task.assign",
        attributes = mapOf("task.title" to p.title),
        causes = listOf("notification.process"),
    ) }
)
```
</TabItem>
...
<TabItem label="Python">
```python
async def create_task(ctx, payload):
    res = await ctx.post("/api/tasks", payload)
    flush_tracing()  # Ensure spans reach the OTLP receiver
    return res.json()
```
</TabItem>
<TabItem label="Ruby">
```ruby
create_task: ->(ctx, payload) {
  res = ctx.post("/api/tasks", payload)
  flush_tracing  # Ensure spans reach the OTLP receiver
  res.body
}
```
...
<TabItem label="Go">
```go
func (a *adapter) CreateTask(ctx context.Context, p CreateTaskPayload) (any, error) {
    res, err := ctx.Post("/api/tasks", p)
    flushTracing() // Ensure spans reach the OTLP receiver
    return res.JSON(), err
}
```
</TabItem>
<TabItem label="Rust">
```rust
async fn create_task(ctx: &mut Ctx, payload: CreateTaskPayload) -> Result<Value> {
    let res = ctx.post("/api/tasks", &payload).await?;
    flush_tracing().await; // Ensure spans reach the OTLP receiver
    Ok(res.json())
}
```
</TabItem>
<TabItem label="Kotlin">
```kotlin
override suspend fun createTask(ctx: Ctx, payload: CreateTaskPayload): Any {
    val res = ctx.post("/api/tasks", payload)
    flushTracing() // Ensure spans reach the OTLP receiver
    return res.json()
}
```
