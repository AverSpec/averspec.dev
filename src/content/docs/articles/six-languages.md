...
The Ruby API rewrite changed every adapter, every test, every example. The Go domain redesign rewired how markers are stored and accessed across 30 files. Python's async handling was redesigned twice. The acceptance tests across every port were migrated from plain assertions to domain-mediated operations — forty files per language.
...
The Aver ports ran in a single conversation with a million-token context window. By the third language, the agent had seen: how Python solved the async/sync boundary (hide it in the proxy), how Ruby's RSpec load-time evaluation broke adapter registration, how Go's parallel test execution raced on environment variables, which simulated panel review caught a Python dataclass flaw before any code was written. It didn't rediscover any of that through error messages. It applied it.
...
But the real leverage came after the initial ports. The DX improvement cycle — auditing every port for language-idiomatic feel, fixing the friction points, re-validating against the spec — happened across five languages simultaneously with minimal effort because every fix was verified instantly. The spec doesn't just enable porting. It enables polishing.

Honest caveat: the spec wasn't free. Three weeks of TypeScript work produced the 534 tests that made everything after it possible. Huntley starts from zero and ships in days. Different trade-off. But once a behavioral spec exists, it amortizes. Every port. Every refactor. Every polish pass. The Ruby DSL-to-classes migration was one dispatch. The Go domain redesign was one dispatch. The Kotlin backtick fix was one line.

## How it played out

Python was first and most hands-on. Design decisions about decorators, async strategy, pytest plugin architecture. A simulated panel of Python experts caught three design flaws before a line of implementation was written.

Ruby was mostly hands-off. Built the core, I checked acceptance count and API feel. Both triggered more work. The agent handled it.

Go, Rust, and Kotlin launched as parallel background agents. Each came back with a working framework, tests, CI, and an example app. The Rust agent solved a borrow checker problem I hadn't anticipated — separating the immutable domain reference from the mutable test context. That's an agent reasoning about a novel constraint with enough context to understand the goal. Not a loop stumbling into a fix.
